...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!path) {
      console.error('[Calendar Image Proxy] Parâmetro path não fornecido');
      return new NextResponse('Parâmetro path é obrigatório', { status: 400 });
    }
    
    console.log(`[Calendar Image Proxy] Caminho original recebido: "${path}"`);
    
    // Validar se o caminho parece ser uma imagem
    const isValidImagePath = /\.(jpe?g|png|webp|gif|svg)$/i.test(path);
    if (!isValidImagePath) {
      console.error(`[Calendar Image Proxy] Caminho não parece ser uma imagem válida: ${path}`);
    ...
    if (!processedPath.startsWith('calendario/')) {
      // Adicionar versão com prefixo 'calendario/'
      const filename = processedPath.split('/').pop() || processedPath;
      pathsToTry.push(`calendario/${filename}`);
    }
    
    console.log(`[Calendar Image Proxy] Caminhos a tentar:`, pathsToTry);
    ...
    for (const testPath of pathsToTry) {
      try {
        console.log(`[Calendar Image Proxy] Tentando buscar: bucket="${process.env.MINIO_BUCKET}", objeto="${testPath}"`);
        ...
        console.log(`[Calendar Image Proxy] Imagem carregada com sucesso: ${testPath}, tamanho: ${fileBuffer.length} bytes`);
        break; // Sair do loop se encontrou o arquivo
      } catch (error) {
        const errorMsg = error instanceof Error ? error.message : String(error);
        console.log(`[Calendar Image Proxy] Falha ao buscar "${testPath}": ${errorMsg}`);
    ...
    console.error(`[Calendar Image Proxy] Não foi possível encontrar o arquivo em nenhum dos caminhos testados`);
    
    // Usar o sistema de fallback para imagens de calendário
    try {
      ...
      if (fallbackResponse) {
        console.log('[Calendar Image Proxy] Servindo imagem de fallback');
        return fallbackResponse;
      }
    } catch (fallbackError) {
      console.error('[Calendar Image Proxy] Erro ao buscar imagem de fallback:', fallbackError);
    }
    
    return new NextResponse('Erro ao carregar imagem, arquivo não encontrado', { status: 404 });
  } catch (error) {
    console.error('[Calendar Image Proxy] Erro interno:', error);
