...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!path) {
      console.error('[Sponsor Image Proxy] Parâmetro path não fornecido');
      return new NextResponse('Parâmetro path é obrigatório', { status: 400 });
    }
    
    console.log(`[Sponsor Image Proxy] Caminho original recebido: "${path}"`);
    
    // Validar se o caminho parece ser uma imagem
    const isValidImagePath = /\.(jpe?g|png|webp|gif|svg)$/i.test(path);
    if (!isValidImagePath) {
      console.error(`[Sponsor Image Proxy] Caminho não parece ser uma imagem válida: ${path}`);
    ...
    if (!processedPath.startsWith('patrocinadores/')) {
      // Adicionar versão com prefixo 'patrocinadores/'
      const filename = processedPath.split('/').pop() || processedPath;
      pathsToTry.push(`patrocinadores/${filename}`);
    }
    
    console.log(`[Sponsor Image Proxy] Caminhos a tentar:`, pathsToTry);
    ...
    for (const testPath of pathsToTry) {
      try {
        console.log(`[Sponsor Image Proxy] Tentando buscar: bucket="${process.env.MINIO_BUCKET}", objeto="${testPath}"`);
        ...
        console.log(`[Sponsor Image Proxy] Imagem carregada com sucesso: ${testPath}, tamanho: ${fileBuffer.length} bytes`);
        break; // Sair do loop se encontrou o arquivo
      } catch (error) {
        const errorMsg = error instanceof Error ? error.message : String(error);
        console.log(`[Sponsor Image Proxy] Falha ao buscar "${testPath}": ${errorMsg}`);
    ...
    console.error(`[Sponsor Image Proxy] Não foi possível encontrar o arquivo em nenhum dos caminhos testados`);
    return new NextResponse('Erro ao carregar imagem, arquivo não encontrado', { status: 404 });
  } catch (error) {
    console.error('[Sponsor Image Proxy] Erro interno:', error);
