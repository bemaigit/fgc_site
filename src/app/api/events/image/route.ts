...
export async function GET(req: NextRequest) {
  try {
    ...
    const type = searchParams.get('type') || 'cover'; // cover ou poster
    
    if (!path) {
      console.error('[Events Image Proxy] Parâmetro path não fornecido');
      return new NextResponse('Parâmetro path é obrigatório', { status: 400 });
    }
    
    console.log(`[Events Image Proxy] Caminho original recebido: "${path}", tipo: ${type}`);
    
    // Validar se o caminho parece ser uma imagem
    const isValidImagePath = /\.(jpe?g|png|webp|gif|svg)$/i.test(path);
    if (!isValidImagePath) {
      console.error(`[Events Image Proxy] Caminho não parece ser uma imagem válida: ${path}`);
    ...
    if (!processedPath.startsWith('eventos/')) {
      // Adicionar versão com prefixo 'eventos/'
      const filename = processedPath.split('/').pop() || processedPath;
      pathsToTry.push(`eventos/${filename}`);
    }
    
    console.log(`[Events Image Proxy] Caminhos a tentar:`, pathsToTry);
    ...
    for (const testPath of pathsToTry) {
      try {
        console.log(`[Events Image Proxy] Tentando buscar: bucket="${process.env.MINIO_BUCKET}", objeto="${testPath}"`);
        ...
        console.log(`[Events Image Proxy] Imagem carregada com sucesso: ${testPath}, tamanho: ${fileBuffer.length} bytes`);
        break; // Sair do loop se encontrou o arquivo
      } catch (error) {
        const errorMsg = error instanceof Error ? error.message : String(error);
        console.log(`[Events Image Proxy] Falha ao buscar "${testPath}": ${errorMsg}`);
    ...
    console.error(`[Events Image Proxy] Não foi possível encontrar o arquivo em nenhum dos caminhos testados`);
    return new NextResponse('Erro ao carregar imagem, arquivo não encontrado', { status: 404 });
  } catch (error) {
    console.error('[Events Image Proxy] Erro interno:', error);
