...
export function LogoUploader({ currentLogo, onLogoChange }: LogoUploaderProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Usa a URL processada para o proxy
      onLogoChange(url)
    ...
    <div className="space-y-4">
      <div className="flex items-center space-x-4">
        ...
        <div className="w-20 h-20 relative border rounded">
          <Image
            src={processHeaderLogoUrl(currentLogo)}
            alt="Logo atual"
            fill
            className="object-contain p-2"
            unoptimized // Adiciona esta prop para imagens externas
          />
        </div>
